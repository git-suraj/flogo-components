...
export class AppendActivityContribution extends WiServiceHandlerContribution {
    ...
    value = (fieldName: string, context: IActivityContribution): Observable<any> | any => {
        ...
        if (fieldName === "output") {
            let inputSchema: IFieldDefinition = context.getField("item");
            console.log("output schema is")
            console.log(inputSchema.value.value)

            let outputSchema = schema.json(JSON.parse(inputSchema.value.value))
